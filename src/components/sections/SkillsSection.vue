...
<template>
  <section id="habilidades" class="section-shell">
      ...
      <MotionReveal class="-mx-5 overflow-x-auto overscroll-x-contain px-5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" :delay="0.08">
        <div class="flex min-w-max gap-2" role="tablist" aria-label="Categorías de habilidades">
          <button
            ...
            class="rounded-full border px-3.5 py-2 text-xs font-semibold whitespace-nowrap transition-all duration-300 hover:-translate-y-0.5 active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-cyan sm:text-sm"
            :class="
              activeCategoryId === category.id
                ? 'border-brand-cyan bg-brand-cyan text-space shadow-lg shadow-brand-cyan/15'
                : 'border-white/10 bg-surface/75 text-muted hover:border-brand-cyan/35 hover:bg-white/4 hover:text-foreground'
            "
